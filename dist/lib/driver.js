...
function login(credentials = {
    ...
    if (credentials.ldap) {
        ...
        log_1.logger.info(`[login] Logging in ${credentials.username} with CROWD`);
        login = exports.ddp.login({ crowd: true, username: credentials.username, crowdPassword: credentials.password });
    }
    else if (credentials.totp) {
        log_1.logger.info(`[login] Logging in ${credentials.username} with TOTP`);
        ...
        log_1.logger.info(`[login] Logging in with resume token`);
        login = exports.ddp.login({ resume: credentials.resume });
    }
    else if (credentials.oauth) {
        log_1.logger.info(`[login] Logging in with OAuth`);
        login = exports.ddp.login({ oauth: credentials.oauth });
    }
    else {
 ...
 */
function subscribe(topic, roomId, ...params) {
    return new Promise((resolve, reject) => {
        log_1.logger.info(`[subscribe] Preparing subscription: ${topic}: ${roomId}`);
        const promiseSubscription = exports.ddp.subscribe(topic, roomId, ...params, true);
...
function on(collection, callback) {
    log_1.logger.info(`[reactive] Listening for change events in collection ${collection}`);
    exports.ddp.on(collection, (result) => {
